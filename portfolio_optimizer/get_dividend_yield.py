...
for ticker in tickers:
    try:
        dividend_rate = yf.Ticker(ticker).info['trailingAnnualDividendYield'] * 100
        print(ticker,dividend_rate)
        cursor.execute(
            "UPDATE sp500_table SET dividend=%s WHERE ticker=%s", (dividend_rate, ticker)
        )
        connection.commit()
